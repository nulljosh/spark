...
- security: scrub committed daemon secret, rotate SPARK_DAEMON_SECRET
- fix: post delete endpoint 500 (bad `_lib` import path); use service role so RLS doesn't drop the delete
- fix: defensive headers access in login rate limiter
- tests: repair stale imports (api/_lib/store), gate server-dependent workflow tests behind API_URL, drop removed comment-counts tests, align seed count (12), invert Base64 backward-compat test to assert rejection

## [v2.0.1] — 2026-05-23

- 5b69566a chore: snow leopard pass (dose)
- f74892b7 chore: snow leopard pass (roost)
